...
func (dq *DiagnosisQuery) QueryDisease() *DiseaseQuery {
...
func (dq *DiagnosisQuery) QueryEmployee() *EmployeeQuery {
...
// First returns the first Diagnosis entity in the query. Returns *NotFoundError when no diagnosis was found.
...
// FirstID returns the first Diagnosis id in the query. Returns *NotFoundError when no id was found.
...
func (dq *DiagnosisQuery) FirstXID(ctx context.Context) int {
...
// Only returns the only Diagnosis entity in the query, returns an error if not exactly one entity was returned.
...
// OnlyID returns the only Diagnosis id in the query, returns an error if not exactly one id was returned.
...
func (dq *DiagnosisQuery) WithDisease(opts ...func(*DiseaseQuery)) *DiagnosisQuery {
	query := &DiseaseQuery{config: dq.config}
...
func (dq *DiagnosisQuery) WithEmployee(opts ...func(*EmployeeQuery)) *DiagnosisQuery {
	query := &EmployeeQuery{config: dq.config}
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(diagnosis.FieldDiagnosticMessages).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
//
//	client.Diagnosis.Query().
//		Select(diagnosis.FieldDiagnosticMessages).
//		Scan(ctx, &v)
//
func (dq *DiagnosisQuery) Select(field string, fields ...string) *DiagnosisSelect {
	selector := &DiagnosisSelect{config: dq.config}
...
func (dq *DiagnosisQuery) sqlAll(ctx context.Context) ([]*Diagnosis, error) {
	...
	if query := dq.withPatient; query != nil {
		...
		nodeids := make(map[int][]*Diagnosis)
		for i := range nodes {
			if fk := nodes[i].patient_diagnosis; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "patient_diagnosis" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := dq.withEmployee; query != nil {
		...
		nodeids := make(map[int][]*Diagnosis)
		for i := range nodes {
			if fk := nodes[i].employee_diagnosis; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "employee_diagnosis" returned %v`, n.ID)
			}
			for i := range nodes {
...
// DiagnosisGroupBy is the builder for group-by Diagnosis entities.
...
// DiagnosisSelect is the builder for select fields of Diagnosis entities.
