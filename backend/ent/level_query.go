...
func (lq *LevelQuery) QueryArea() *AreaQuery {
	query := &AreaQuery{config: lq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := lq.prepareQuery(ctx); err != nil {
			return nil, err
		}
		step := sqlgraph.NewStep(
			sqlgraph.From(level.Table, level.FieldID, lq.sqlQuery()),
...
func (lq *LevelQuery) FirstXID(ctx context.Context) int {
...
func (lq *LevelQuery) WithArea(opts ...func(*AreaQuery)) *LevelQuery {
	query := &AreaQuery{config: lq.config}
...
//
//	client.Level.Query().
//		Select(level.FieldLevelName).
//		Scan(ctx, &v)
//
func (lq *LevelQuery) Select(field string, fields ...string) *LevelSelect {
	selector := &LevelSelect{config: lq.config}
...
func (lq *LevelQuery) sqlAll(ctx context.Context) ([]*Level, error) {
	...
	if query := lq.withArea; query != nil {
		...
		query.Where(predicate.Area(func(s *sql.Selector) {
			s.Where(sql.InValues(level.AreaColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.level_area
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "level_area" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "level_area" returned %v for node %v`, *fk, n.ID)
			}
...
func (lq *LevelQuery) sqlQuery() *sql.Selector {
